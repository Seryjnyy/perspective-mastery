...
import { LookAtMode } from "@/app/newtesting/scene-store/lookat-slice";
...
export const ControlPanelCameraTab = () => {
  ...
  const lookAtTargetInSceneDefaults = useTestingNewStore(
    (state) => state.lookAtTarget.defaultsInScene
  );

  const setLookAtTargetPosition = useTestingNewStore(
    (state) => state.setLookAtTargetPosition
  );
  // TODO : there should be specific actions for look-at target
      ...
      <Tabs defaultValue="camera">
        ...
        <TabsContent value="look-at">
          ...
          <ControlPanelSlider
            label="Target X"
            value={
              lookAtTarget.inScene?.position?.x ||
              lookAtTargetInSceneDefaults.position.x
            }
            min={-5}
            max={5}
            step={0.1}
            onChange={(value) =>
              setLookAtTargetPosition({
                ...(lookAtTarget.inScene.position ||
                  lookAtTargetInSceneDefaults.position),
                x: value,
              })
            }
          ...
          <ControlPanelSlider
            label="Target Y"
            value={
              lookAtTarget.inScene?.position?.y ||
              lookAtTargetInSceneDefaults.position.y
            }
            min={-5}
            max={5}
            step={0.1}
          ...
          <ControlPanelSlider
            label="Target Z"
            value={
              lookAtTarget.inScene?.position?.z ||
              lookAtTargetInSceneDefaults.position.z
            }
            min={-5}
            max={5}
            step={0.1}
